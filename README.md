_Примечание: github сломал половину ссылок оглавления_

### Оглавление

- #### [Ticket 2: Basic Syntax](#ticket-2-basic-syntax)
	- [Ticket 2 Введение](#ticket-2-введение)
	- [Арифметика и типы](#арифметика-и-типы)
	- [Функции](#функции)
	- [Операторы](#операторы)
	- [Тип List](#тип-list)
	- [Конструкции и выражения](#конструкции-и-выражения)
	...
	- [Расширения языка](#расширения-языка)
	- [Применение функций](#применение-функций)
	- [Оператор композиции `.`](#оператор-композиции-)
	- [Генерация списков](#генерация-списков)
	- [Ленивое вычисление](#ленивое-вычисление)
- #### [Ticket 3: Datas, Classes, Instances](#ticket-3-datas-classes-instances)
	- [Ticket 3 Введение](#ticket-3-введение)
	- [Custom Types (Пользовательские типы)](#custom-types-пользовательские-типы)
	- [ADT (Алгебраические типы данных)](#adt-алгебраические-типы-данных)
	- [Record Types (Типы-записи)](#record-types-типы-записи)
	- [Type Classes (Классы типов)](#type-classes-классы-типов)
	- [Модули](#модули)
	- [Дополнительные темы](#дополнительные-темы)
	- [Ticket 3 Итог](#ticket-3-итог)
- #### [Ticket 4: Basic typeclasses: Monoid, Functor, Applicative](#ticket-4-basic-typeclasses-monoid-functor-applicative)
	- [1. Type-classes](#1-type-classes)
		- [1.1. Semigroup (Полугруппа)](#11-semigroup-полугруппа)
		- [1.2. Monoid (Моноид)](#12-monoid-моноид)
	- [2. Свёртка (Fold)](#2-свёртка-fold)
		- [2.1. `foldr` и `foldl`](#21-foldr-и-foldl)
		- [2.2. Класс `Foldable`](#22-класс-foldable)
	- [3. Functor](#3-functor)
	- [4. Applicative](#4-applicative)
	- [5. Alternative](#5-alternative)
	- [6. Traversable](#6-traversable)
	- [7. Фантомные типы](#7-фантомные-типы)
	- [8. Расширения](#8-расширения)
		- [8.1. `ScopedTypeVariables`](#81-scopedtypevariables)
		- [8.2. `TypeApplications`](#82-typeapplications)
		- [8.3. `AllowAmbiguousTypes`](#83-allowambiguoustypes)
- #### [Ticket 5: Monads](#ticket-5-monads)
	- [Введение в монады](#введение-в-монады)
	- [Примеры монад](#примеры-монад)
		- [1. Maybe Monad](#1-maybe-monad)
		- [2. Either Monad](#2-either-monad)
		- [3. List Monad](#3-list-monad)
	- [Композиция монад](#композиция-монад)
	- [Операторы и функции](#операторы-и-функции)
		- [1. Оператор `>>` (Zen)](#1-оператор-zen)
		- [2. Функция `join`](#2-функция-join)
	- [Монадические законы](#монадические-законы)
	- [Специализированные монады](#специализированные-монады)
		- [1. Writer Monad](#1-writer-monad)
		- [2. Reader Monad](#2-reader-monad)
		- [3. State Monad](#3-state-monad)
	- [Дополнительные темы](#дополнительные-темы)
- #### [Ticket 6: RealWorld](#ticket-6-realworld)
	- [1. `getchar` и проблемы чистых функций](#1-getchar-и-проблемы-чистых-функций)
	- [2. `RealWorld` и модель `IO`](#2-realworld-и-модель-io)
	- [3. `do`-нотация и порядок выполнения](#3-do-нотация-и-порядок-выполнения)
	- [4. Работа с вводом-выводом](#4-работа-с-вводом-выводом)
	- [5. Ленивое IO и проблемы](#5-ленивое-io-и-проблемы)
	- [6. FFI (Foreign Function Interface)](#6-ffi-foreign-function-interface)
	- [7. Изменяемые объекты](#7-изменяемые-объекты)
	- [8. Исключения](#8-исключения)
	- [9. Класс `Exception`](#9-класс-exception)
	- [10. Полезные функции для работы с исключениями](#10-полезные-функции-для-работы-с-исключениями)
	- [11. Небезопасные операции (`unsafePerformIO`)](#11-небезопасные-операции-unsafeperformio)
	- [12. Работа со строками `Text` и `ByteString`](#12-работа-со-строками-text-и-bytestring)
